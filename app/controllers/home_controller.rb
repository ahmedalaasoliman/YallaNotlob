class HomeController < ApplicationController
  def index  
	...
	#@friend=
	@friend_orders=Order.find_by_sql(["select name , order_for from orders , friends  , users  where friends.user_id= ? and users.id = friends.friend and friends.friend = users.id" , @user_id]).last(5)
  end
end
